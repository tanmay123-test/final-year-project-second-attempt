...
class EnhancedFinny:
    ...
    def show_menu(self, user_id):
        while True:
            ...
            print("="*60)
            print("1.    Conversational Entry (Recommended)")
    ...
    def _monthly_summary(self, user_id):
        ...
        # Top category
        if summary:
            top_item = max(summary, key=lambda x: x[1])
            print(f"  Top spending: {top_item[0].title()} ( {top_item[1]:.2f})")
            
            # Daily average for top category
            daily_avg = top_item[1] / 30
            print(f"  Daily {top_item[0]} average:  {daily_avg:.2f}")
