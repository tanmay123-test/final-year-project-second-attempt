...
def trip_planner_menu(user_id, token):
    ...
    while True:
        ...
        print("="*60)
        print("1. 🗺️  Plan New Trip")
        print("2. 📋 Trip History")
        print("3. ⚙️  Settings")
        print("4. ⬅️  Back to Car Service")
...
def display_trip_plan(trip_plan):
    ...
    print(f"   🛣️  Distance: {trip_plan.get('distance_km', 0)} km")
    print(f"   ⏱️  ETA: {trip_plan.get('duration_hours', 0)} hours")
    print("")
    print("💰 COST BREAKDOWN:")
    print(f"   ⛽ Fuel needed: {trip_plan.get('fuel_needed', 0)} liters")
    print(f"   💸 Fuel cost: ₹{trip_plan.get('fuel_cost', 0)}")
    print(f"   🛣️  Estimated toll: ₹{trip_plan.get('toll_cost', 0)}")
