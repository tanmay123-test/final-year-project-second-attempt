...
def fuel_delivery_user_interface(user_info):
    ...
    for i, agent in enumerate(nearby_agents, 1):
        vehicle_emoji = "🏍️" if agent['vehicle_type'] == 'Bike' else "🚐" if agent['vehicle_type'] == 'Van' else "🚚"
        ...
        print(f"   🗺️ Service Area: {agent['service_area_km']} km")
        print()
...
def create_fuel_request_manual(user_info, fuel_type, fuel_quantity, location_name, selected_agent):
    ...
    print(f"⏱️ ETA: {selected_agent['eta_minutes']} minutes")
