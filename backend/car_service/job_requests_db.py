...
# Database path
JOB_REQUESTS_DB = os.path.join(os.path.dirname(__file__), 'job_requests.db')

class JobRequestsDB:
    def __init__(self):
        self.conn = sqlite3.connect(JOB_REQUESTS_DB, check_same_thread=False)
        self.conn.row_factory = sqlite3.Row
        self.create_table()
    
    def get_conn(self):
        return sqlite3.connect(JOB_REQUESTS_DB, check_same_thread=False)
    ...
    def create_table(self):
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS mechanic_stats (
                ...
                fairness_score REAL DEFAULT 100,
    ...
    def get_pending_jobs(self, mechanic_id: int) -> List[Dict]:
        ...
        cursor.execute("""
            ...
            WHERE mechanic_id = ? AND status = 'ASSIGNED'
    ...
    def get_job_request(self, job_id: int) -> Optional[Dict]:
        ...
        cursor = self.conn.cursor()
        cursor.execute("SELECT * FROM job_requests WHERE id = ?", (job_id,))
        row = cursor.fetchone()
    ...
    def reject_job(self, job_id: int, reason: str = None) -> bool:
        ...
        cursor.execute("""
            UPDATE job_requests 
            SET status = 'REJECTED', updated_at = ?, assignment_reason = ?
            WHERE id = ? AND status = 'ASSIGNED'
    ...
    def get_expired_jobs(self) -> List[Dict]:
        ...
        cursor.execute("""
            SELECT * FROM job_requests 
            WHERE status = 'ASSIGNED' AND response_deadline < ?
        """, (current_time,))
        return [dict(row) for row in cursor.fetchall()]
    ...
    def reassign_job(self, job_id: int, new_mechanic_id: int, reason: str = None) -> bool:
        ...
        cursor.execute("""
            UPDATE job_requests 
            SET mechanic_id = ?, status = 'ASSIGNED', updated_at = ?, 
                response_deadline = ?, assignment_reason = ?
            WHERE id = ?
    ...
    def get_job_statistics(self, mechanic_id: int) -> Dict:
        ...
        cursor.execute("""
            ...
            WHERE mechanic_id = ? AND status = 'COMPLETED' AND DATE(created_at) = ?
        """, (mechanic_id, today))
    ...
    def get_all_jobs(self, status: str = None) -> List[Dict]:
        ...
        cursor = self.conn.cursor()
        if status:
            cursor.execute("SELECT * FROM job_requests WHERE status = ? ORDER BY created_at DESC", (status,))
        else:
    ...
    def get_active_job(self, mechanic_id: int) -> Optional[Dict]:
        ...
        cursor.execute("""
            ...
            WHERE aj.mechanic_id = ? AND aj.status IN ('ARRIVING', 'ARRIVED', 'WORKING')
    ...
    def update_active_job_status(self, active_job_id: int, status: str, **kwargs) -> bool:
        ...
        if 'arrival_time' in kwargs:
            update_fields.append("arrival_time = ?")
        ...
        if 'after_photo' in kwargs:
            update_fields.append("after_photo = ?")
    ...
    def verify_otp(self, active_job_id: int, otp: str) -> bool:
        ...
        cursor = self.conn.cursor()
        cursor.execute("SELECT otp FROM active_jobs WHERE id = ?", (active_job_id,))
        row = cursor.fetchone()
    ...
    def calculate_repair_time(self, active_job_id: int) -> str:
        ...
        cursor = self.conn.cursor()
        cursor.execute("""
            SELECT start_time FROM active_jobs WHERE id = ? AND status = 'WORKING'
        """, (active_job_id,))
        row = cursor.fetchone()
    ...
    def get_mechanic_earnings_summary(self, mechanic_id: int) -> Dict:
        ...
        return {
            'today_earnings': today_data['today_earnings'],
            'today_jobs': today_data['today_jobs'],
            'today_bonus': today_data['today_bonus'],
            'today_commission': today_data['today_commission'],
            'total_earnings': total_data['total_earnings'],
            'total_jobs': total_data['total_jobs'],
            'total_bonus': total_data['total_bonus'],
            'total_commission': total_data['total_commission']
        }
    ...
    def get_mechanic_stats(self, mechanic_id: int) -> Dict:
        ...
        cursor = self.conn.cursor()
        cursor.execute("""
            SELECT * FROM mechanic_stats WHERE mechanic_id = ?
        """, (mechanic_id,))
    ...
    def update_mechanic_stats(self, mechanic_id: int, job_completed: bool = True):
        ...
        cursor.execute("""
            ...
            SET total_jobs = ?, completed_jobs = ?, cancelled_jobs = ?,
                acceptance_rate = ?, completion_rate = ?, fairness_score = ?,
                total_earnings = ?, last_job_time = ?, updated_at = ?
            WHERE mechanic_id = ?
        """, (total_jobs, completed_jobs, cancelled_jobs, acceptance_rate, 
              completion_rate, fairness_score, total_earnings, 
              datetime.now().isoformat(), datetime.now().isoformat(), mechanic_id))
    ...
    def get_mechanic_performance(self, mechanic_id: int) -> Dict:
        ...
        cursor = self.conn.cursor()
        cursor.execute("""
            SELECT * FROM mechanic_performance WHERE mechanic_id = ?
        """, (mechanic_id,))
    ...
    def initialize_mechanic_performance(self, mechanic_id: int):
        ...
        cursor = self.conn.cursor()
        cursor.execute("""
            INSERT OR REPLACE INTO mechanic_performance 
    ...
    def update_mechanic_performance(self, mechanic_id: int, job_completed: bool = True, 
        ...
        cursor.execute("""
            ...
            WHERE mechanic_id = ?
        """, (total_jobs, completed_jobs, cancelled_jobs, acceptance_rate, 
              completion_rate, avg_response_time, datetime.now().isoformat(), mechanic_id))
