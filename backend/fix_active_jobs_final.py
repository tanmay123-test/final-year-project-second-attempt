#!/usr/bin/env python3
"""
Final fix for active jobs - move accepted job to active_jobs table
"""
...
smart_search_db_path = os.path.join(os.path.dirname(__file__), 'car_service', 'smart_search.db')

try:
    # Connect to databases
    booking_conn = sqlite3.connect(booking_db_path)
    active_conn = sqlite3.connect(active_jobs_db_path)
    smart_conn = sqlite3.connect(smart_search_db_path)
    
    # Create cursors
    booking_cursor = booking_conn.cursor()
    active_cursor = active_conn.cursor()
    location_cursor = smart_conn.cursor()
    ...
    if accepted_job:
        ...
        if location_data:
            ...
            print("   Mechanic location not found in smart_search_db")
            mechanic_lat, mechanic_long = 19.0954, 72.8783
        
        # Create job_requests entry first
        active_cursor.execute(
            "INSERT INTO job_requests (user_id, mechanic_id, issue, issue_type, car_model, estimated_earning, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            (accepted_job[1], accepted_job[2], accepted_job[4], 'Unknown Car', accepted_job[12], 'ACCEPTED', 'Sarthy', datetime.now().isoformat())
        )
        ...
        active_cursor.execute(
            "INSERT OR IGNORE INTO active_jobs (job_request_id, user_id, mechanic_id, status, created_at) VALUES (?, ?, ?, ?)",
