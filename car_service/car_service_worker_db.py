...
class CarServiceWorkerDB:
    ...
    def get_worker_by_email(self, email: str) -> Optional[Dict]:
        """Get worker by email"""
        cursor = self.conn.cursor()
        cursor.execute("SELECT * FROM car_service_workers WHERE email = ?", (email,))
        row = cursor.fetchone()
    ...
    def get_worker_by_phone(self, phone: str) -> Optional[Dict]:
        ...
        cursor = self.conn.cursor()
        cursor.execute("SELECT * FROM car_service_workers WHERE phone = ?", (phone,))
        row = cursor.fetchone()
        return dict(row) if row else None
    ...
    def get_worker_by_id(self, worker_id: int) -> Optional[Dict]:
        """Get worker by ID"""
        cursor = self.conn.cursor()
        cursor.execute("SELECT * FROM car_service_workers WHERE id = ?", (worker_id,))
        row = cursor.fetchone()
        return dict(row) if row else None
    ...
    def get_mechanics(self, status: str = None) -> List[Dict]:
        ...
        cursor = self.conn.cursor()
        if status:
            cursor.execute("SELECT * FROM car_service_workers WHERE role = 'Mechanic' AND status = ? ORDER BY created_at DESC", (status,))
        else:
    ...
    def get_fuel_agents(self, status: str = None) -> List[Dict]:
        ...
        cursor = self.conn.cursor()
        if status:
            cursor.execute("SELECT * FROM car_service_workers WHERE role = 'Fuel Delivery Agent' AND status = ? ORDER BY created_at DESC", (status,))
        else:
    ...
    def get_tow_operators(self, status: str = None) -> List[Dict]:
        ...
        cursor = self.conn.cursor()
        if status:
            cursor.execute("SELECT * FROM car_service_workers WHERE role = 'Tow Truck Operator' AND status = ? ORDER BY created_at DESC", (status,))
        else:
    ...
    def get_automobile_experts(self, status: str = None) -> List[Dict]:
        ...
        cursor = self.conn.cursor()
        if status:
            cursor.execute("SELECT * FROM car_service_workers WHERE role = 'Automobile Expert' AND status = ? ORDER BY created_at DESC", (status,))
        else:
    ...
    def get_online_mechanics(self) -> List[Dict]:
        ...
        cursor = self.conn.cursor()
        cursor.execute("SELECT * FROM car_service_workers WHERE role = 'Mechanic' AND is_online = 1 AND status = 'APPROVED'")
        return [dict(row) for row in cursor.fetchall()]
    
    def get_online_fuel_agents(self) -> List[Dict]:
        """Get all online fuel delivery agents"""
        cursor = self.conn.cursor()
        cursor.execute("SELECT * FROM car_service_workers WHERE role = 'Fuel Delivery Agent' AND is_online = 1 AND status = 'APPROVED'")
        return [dict(row) for row in cursor.fetchall()]
    
    def get_online_tow_operators(self) -> List[Dict]:
        """Get all online tow truck operators"""
        cursor = self.conn.cursor()
        cursor.execute("SELECT * FROM car_service_workers WHERE role = 'Tow Truck Operator' AND is_online = 1 AND status = 'APPROVED'")
        return [dict(row) for row in cursor.fetchall()]
    
    def get_online_experts(self) -> List[Dict]:
        """Get all online automobile experts"""
        cursor = self.conn.cursor()
        cursor.execute("SELECT * FROM car_service_workers WHERE role = 'Automobile Expert' AND is_online = 1 AND status = 'APPROVED'")
        return [dict(row) for row in cursor.fetchall()]
    ...
    def get_workers_by_role(self, role: str, status: str = None) -> List[Dict]:
        ...
        cursor = self.conn.cursor()
        if status:
            cursor.execute("SELECT * FROM car_service_workers WHERE role = ? AND status = ? ORDER BY created_at DESC", (role, status))
        else:
            cursor.execute("SELECT * FROM car_service_workers WHERE role = ? ORDER BY created_at DESC", (role,))
        return [dict(row) for row in cursor.fetchall()]
