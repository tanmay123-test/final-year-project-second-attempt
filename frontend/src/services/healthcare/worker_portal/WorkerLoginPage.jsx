...
import { ArrowLeft, Mail, Stethoscope, Loader2 } from 'lucide-react';
...
const WorkerLoginPage = () => {
  ...
  const onLogin = async (e) => {
    ...
    setError('');
    try {
      const { data } = await api.post('/worker/login', { email });
      const workerId = data.worker_id || data.id;
      const token = data.token || data.worker_token || data.access_token;
      if (!workerId || !token) throw new Error('Invalid login response');

      localStorage.setItem('worker_id', String(workerId));
      localStorage.setItem('worker_token', String(token));
      if (data.specialization) localStorage.setItem('worker_specialization', data.specialization);
      navigate('/worker/dashboard');
    } catch (err) {
      setError(err?.response?.data?.error || 'Worker not found. Please check your email or sign up.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="wp-page">
      <div className="wp-mobile-shell">
        ...
        <div className="wp-form-card wp-card" style={{ padding: 24 }}>
          <div style={{ width: 56, height: 56, margin: '0 auto', borderRadius: '50%', background: '#EFF6FF', color: '#1F5F7A', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <Stethoscope size={28} />
          </div>
          ...
          <div style={{ marginTop: 14, fontSize: 14, color: '#64748b', textAlign: 'center' }}>
            New here?{' '}
            <button onClick={() => navigate('/worker/signup')} style={{ border: 'none', background: 'none', color: '#1F5F7A', textDecoration: 'underline', cursor: 'pointer' }}>
